...
export class ReservationHoldRepository {
  ...
  async findActive(experienceId?: string, departureId?: string) {
  ...
  async create(data: Prisma.ReservationHoldCreateInput): Promise<ReservationHold> {
    return prisma.reservationHold.create({ data });
  }

  async update(id: string, data: Prisma.ReservationHoldUpdateInput): Promise<ReservationHold> {
    return prisma.reservationHold.update({ where: { id }, data });
  }

  async releaseExpired(id: string): Promise<ReservationHold> {
  ...
  async findByStatus(status: HoldStatus, limit = 100) {
    return prisma.reservationHold.findMany({
      where: { status },
      orderBy: { createdAt: "desc" },
      take: limit,
