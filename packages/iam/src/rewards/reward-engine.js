...
export class RewardEngine {
    ...
    start() {
        if (this.isRunning)
            return;
        this.isRunning = true;
        this.unsubscribeBookingCompleted = eventBus.on("booking.completed", this.handleBookingCompleted);
    }
    stop() {
        if (!this.isRunning)
            return;
    ...
    unsubscribeBookingCompleted = null;
    handleBookingCompleted = async (event) => {
        try {
    ...
    async processBookingCompletion(event) {
        ...
        await rewardService.createRewardTransaction(event.partnerId, event.bookingId, rule.id, rule.pointsPerBooking, rule.cashMultiplier ? Number(rule.cashMultiplier) : 0, rule.currency ?? "KES", description);
    }
}
