...
export class OtpService {
   ...
   */
  async generateLoginOtp(identifier: string): Promise<{ expiresAt: string }> {
  ...
  async verifyLoginOtp(
    ...
    const loginSuccessPayload: { userId: string; email?: string; phone?: string } = { userId: user.id };
    if (user.email) loginSuccessPayload.email = user.email;
    if (user.phone) loginSuccessPayload.phone = user.phone;
    await auditService.logLoginSuccess(user.id, loginSuccessPayload);
