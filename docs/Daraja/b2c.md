...
By Safaricom
Enables a business to send money to a registered mobile money customer.
...
How It Works
The Merchant (Partner) prepares and sends a payment request to the M-PESA B2C API endpoint.
The API Management Platform validates, authorizes, and authenticates the request, forwards it to M-PESA, and sends an acknowledgment to the Merchant.
M-PESA validates the request and initiator details, then processes the transaction.
M-PESA sends the transaction response to the Merchant via the specified callback URL.
The Customer receives an SMS notification confirming the payment
...
Integration Steps
Sequence Diagram
business-to-customer-sequence-diagram!
...
Create a new test app under apps on the main nerve bar, select business to customer product. Once app is successfully created the simulator is automated to pick app credentials (Consumer key and Consumer Secret) and predefined test data, you can hit the simulate button. Create App!
